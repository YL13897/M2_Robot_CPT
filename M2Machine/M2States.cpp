...
void M2ProbMoveState::duringCode() {
    ...
    switch (currentPhase) {
        ...
        case TRIAL: {
            ...
            bool reachedC = (n < epsC) && (tTrial >= minTrialEvalTime) && X(1) < 0.28; // also require not too high (y<28cm)
            ...
            const double k_wall = 2000.0; // wall stiffness N/m
            const double d_wall = 40.0;  // wall damping N·s/m
            const double y_max = 0.30;   // upper boundary (m)
...
void M2ProbMoveState::loadPerturbationForces() {
    ...
    for(int i=0;i<500;i++){
        ...
        leftPerturbForce.push_back(F_const_left);
    }
