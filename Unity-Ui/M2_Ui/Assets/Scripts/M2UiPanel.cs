...
namespace CORC.Demo
{
    public class M2UiPanel : MonoBehaviour
    {
        ...
        public TMP_InputField upForceInput;
        public Button applyForceBtn;


        [Header("Manual kd hold Command")]
        public TMP_InputField kholdInput;
        public TMP_InputField dholdInput;
        public Button applyHoldKDBtn;



        [Header("Home Position Check")]
        // public Vector2 homePos = new Vector2(0.32f, 0.25f); // A point position (m)
        // public float homePosTolerance = 0.05f;          // position default tolerance (m)
        // public float homeVelTolerance = 0.01f;          // reset velocity default tolerance (m/s)
        ...
        // Safely start rest countdown, stopping any existing one
        private void StartRestCountdownSafe()
        {
            if (restCountdownCo != null) StopCoroutine(restCountdownCo);

            restCountdownCo = StartCoroutine(SessionRestCountdown(restSeconds));
        }


        // Reset end markers at the start of a new run
        private void ResetEndMarkersForNewRun()
        {
            if (endMarkerToggle) enableEndMarkers = endMarkerToggle.isOn;

            if (endMarker)
            {
                endMarker.SetEnabled(enableEndMarkers);
                // If enabling, ensure active
                if (!endMarker.gameObject.activeSelf) endMarker.gameObject.SetActive(true);
        ...
        // Wait until the robot is at home position A (within tolerances) or timeout
        // private IEnumerator WaitUntilAtHome()
        // {
        //     float waited = 0f;
        //     while (waited < maxHomeWait)
        //     {
        //         if (proxy != null && proxy.IsReady)
        //         {
        //             var x = proxy.X;   // [x,y]
        //             var v = proxy.dX;  // [vx,vy]
        //             if (x != null && x.Length >= 2 && v != null && v.Length >= 2)
        //             {
        //                 var pos = new Vector2((float)x[0], (float)x[1]);
        //                 // var vel = new Vector2((float)v[0], (float)v[1]);
        //                 bool posOk = Vector2.Distance(pos, homePos) <= homePosTolerance;
        //                 // bool velOk = vel.magnitude <= homeVelTolerance;
        //                 // if (posOk && velOk) yield break; 
        //                 if (posOk) yield break;
        //             }
        //         }
        //         yield return null;
        //         waited += Time.deltaTime;
        //     }
        //     Debug.LogWarning($"[UI] WaitUntilAtHome timeout after {waited:F2}s");
        // }



        // // Reset to A and start trial after countdown
        // private IEnumerator ResetAndStartTrial(int seconds)
        // {
        //     if (proxy != null && proxy.IsReady)
        //     {
        //         proxy.SendCmd("RSTA");
        //         if (statusTxt) statusTxt.text = "Resetting to A...";
        //         yield return StartCoroutine(WaitUntilAtHome());
        //         if (settleAfterRsta > 0)
        //             yield return new WaitForSeconds(settleAfterRsta); // 额外缓冲
        //     }
        //     ResetEndMarkersForNewRun();
        //     yield return StartCoroutine(CountdownAndStartTrial(seconds));
        // }
        ...
        {
            // if (proxy != null && proxy.IsReady){proxy.SendCmd("RSTA"); } // Start fresh with RSTA

            // Reset end markers at experiment start
            if (endMarkerToggle) enableEndMarkers = endMarkerToggle.isOn;
            ...
            if (enableEndMarkers)
            {
                if (endMarker)
                {
                    ...
                    if (!endMarker.gameObject.activeSelf) endMarker.gameObject.SetActive(true);
                    endMarker.HideAll(); // clear old markers
                }
                else
                {
                    ClearEndMarkers();
            ...
            {
                if (endMarker) endMarker.HideAll(); else ClearEndMarkers();
            }
        ...
        private IEnumerator CountdownAndStartTrial(int seconds)
        {
            ...
            // yield return new WaitForSeconds(0.2f); // 推迟 GO! 的显示
            if (countdownText) countdownText.text = "GO!";
            // yield return new WaitForSeconds(0.2f);// 发送 STRT（只发一次）
            proxy.SendCmd("STRT");
        ...
        private IEnumerator AutoSessionRoutine()
        {
            ...
            for (int g = 1; g <= numGroups && !stopAutoRequested; g++)
            {
                ...
                for (int t = 1; t <= trialsPerGroup && !stopAutoRequested; t++)
                {
                    ...
                    // // 3) Countdown → STRT
                    // // yield return StartCoroutine(CountdownAndStartTrial(Mathf.RoundToInt(preTrialCountdown)));
                    // float waited_for_A = 0f;
                    // while (!readyAtHome && waited_for_A < maxHomeWait) {yield return null; waited_for_A += Time.deltaTime; }
                    // yield return StartCoroutine(CountdownAndStartTrial(Mathf.RoundToInt(preTrialCountdown)));

                    // // 4) Wait for TREN/timeout
                    // lastTrialEnded = false;
                    // float safetyTimeout = 10f;
                    // float waited = 0f;
                    // while (!lastTrialEnded && !stopAutoRequested && waited < safetyTimeout)
        ...
        public void OnFinishClick()
        {
            ...
            // Stop auto session if running
            stopAutoRequested = true;
            autoRunning = false;
            lastTrialEnded = true; // in case waiting for trial end

            if (proxy != null && proxy.IsReady)
            {
                proxy.SendCmd("HALT");
                if (statusTxt) statusTxt.text = "Experiment Finished.";
                Debug.Log("[UI] Sent: HALT");
            }
            ResetEndMarkersForNewRun(); // Clear markers at finish
        ...
        private const string ForceCmd = "FRC2";   // Command to send 2D force
        private const double ForceFallbackLeft = -30.0; // default left force if parsing fails
        private const double ForceFallbackUp = 50.0; // default up force if parsing fails
        private const double ForceLimitLeft = 80.0; // max abs force allowed
        private const double ForceLimitUp = 80.0; // max abs force allowed

        private static double ClampForce(double value, double ForceLimit = 80.0)
        {
            return Math.Max(-ForceLimit, Math.Min(ForceLimit, value));
        }


        
        private void OnApplyForceCommand() // Send manual force command
        {
            if (proxy == null || !proxy.IsReady)
            {
                Debug.LogWarning("[UI] Proxy not ready; skip force command");
                return;
            }

            double left, up;
            bool leftOk = TryParse(leftForceInput ? leftForceInput.text : null, out left, ForceFallbackLeft);
            bool upOk = TryParse(upForceInput ? upForceInput.text : null, out up, ForceFallbackUp);

            left = ClampForce(left, ForceLimitLeft);
            up = ClampForce(up, ForceLimitUp);

            proxy.SendCmd(ForceCmd, new[] { up, left });
            Debug.Log($"[UI] Sent {ForceCmd}: up={up:F3}, left={left:F3}");

            if (statusTxt)
                statusTxt.text = $"Force cmd -> U:{up:F2}, L:{left:F2}";
        }




        private const string holdKDCmd = "HLKD";   // Command to send hold at A K&D
        private const double default_k_hold = 350; 
        private const double default_d_hold = 35; 
        private const double k_hold_limit = 2000; 
        private const double d_hold_limit = 200; 


        private void OnApplyHoldKDCommand()
        {
            if (proxy == null || !proxy.IsReady)
            {
                Debug.LogWarning("[UI] Proxy not ready; skip hold KD command");
                return;
            }

            TryParse(kholdInput ? kholdInput.text : null, out var kHold, default_k_hold);
            TryParse(dholdInput ? dholdInput.text : null, out var dHold, default_d_hold);
            kHold = Math.Max(0, Math.Min(k_hold_limit, kHold));
            dHold = Math.Max(0, Math.Min(d_hold_limit, dHold));

            proxy.SendCmd(holdKDCmd, new[] { kHold, dHold });
            Debug.Log($"[UI] Sent {holdKDCmd}: K={kHold:F3}, D={dHold:F3}");

            if (statusTxt)
                statusTxt.text = $"Hold at A: K & D cmd -> K:{kHold:F2}, D:{dHold:F2}";
        }
        ...
        void Awake()
        {
            ...
            if (!endMarker) endMarker = FindFirstObjectByType<EndMarkerController>();

            if (applyForceBtn) applyForceBtn.onClick.AddListener(OnApplyForceCommand);
        ...
        void Update()
        {
            ...
            foreach (var c in cmds)
            {
                ...
                else if (cmd == "TREN")
                {
                    ...
                    if (scoreTxt)
                    {
                        // Protpcol: cur is at p[7], max at p[8]; fallback to lastMaxTrials if missing
                ...
                else if (cmd == "SESS")
                {
                    ...
                    //     restCountdownCo = StartCoroutine(SessionRestCountdown(restSeconds));
                    // }
                    if (restSeconds > 0) // Always rest after session end unless suppressed
                    {
                        StartRestCountdownSafe();
                    }
                    else
                    {
                ...
                {
                    // When end marker disabled, clear any existing markers and continue
                    if (!enableEndMarkers)
                    {
                        if (endMarker) endMarker.HideAll(); else ClearEndMarkers();
                        continue;
                    ...
                    // params: trialIdx, endX, endY, distToGoal, isTimeout
                    if (p.Length < 5)
                    {
                    ...
                    // Show end marker at the specified position
                    if (endMarker)
                    {
                        endMarker.SetEnabled(true);
                        endMarker.HideAll();
                        endMarker.ShowAtRobot(endX, endY);
                    }
                    else
                    {
                        ClearEndMarkers();
